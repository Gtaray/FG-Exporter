...
namespace Tests
{
    ...
    public class CoreRPG
    {
        ...
        [TestMethod]
        // Tests exporting tokens that are specified in the export window, not on an npc
        public void ExportSpecificTokenImages()
        {
            string modfile = "files/output/corerpg_tokens_specified.mod";
            string controlFolder = "files/corerpg/modules/Tokens, Specified";
            string workingdir = "files/output/corerpg_tokens_specified";
            string jsonfile = "files/corerpg/config/Tokens, Specified.json";
            ...
            using (var zip = new ZipArchive(file, ZipArchiveMode.Read))
            {
                Assert.AreEqual(zip.Entries.Count, 5);
                Assert.IsTrue(zip.Entries.Any(e => e.FullName == "db.xml"));
                Assert.IsTrue(zip.Entries.Any(e => e.FullName == "definition.xml"));
                Assert.IsTrue(zip.Entries.Any(e => e.FullName == "tokens/Griffin.png"));
                Assert.IsTrue(zip.Entries.Any(e => e.FullName == "tokens/Giant Crocodile.png"));
